...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
                ...
                <div class="sm:fixed sm:top-0 sm:right-0 p-6 text-right">
                    @auth
                        ...
                        <a href="{{ url('/dashboard') }}" class="font-semibold text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white focus:outline focus:outline-2 focus:rounded-sm focus:outline-red-500">Dashboard</a>
                            ...
                            <a href="{{ route('profile.edit') }}" class="ml-4 font-semibold text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white focus:outline focus:outline-2 focus:rounded-sm focus:outline-red-500">Profile</a>
    ...
    {{ csrf_field() }}
            ...
            <div class="max-w-7xl mx-auto p-6 lg:p-8">
                ...
                <?php $days_count = Carbon\Carbon::parse($movie->created_at)->diffInDays($now); ?> 
                <div class="mt-16 justify-center" >
                    <div class="grid grid-cols-1 md:grid-cols-1 ">
                        <div  class="scale-100 p-6 bg-white dark:bg-gray-800/50 dark:bg-gradient-to-bl from-gray-700/50 via-solid dark:ring-1 dark:ring-inset dark:ring-white/5 rounded-lg shadow-2xl shadow-gray-500/20 dark:shadow-none flex motion-safe:hover:scale-[1.01] transition-all duration-250 focus:outline focus:outline-2 focus:outline-red-500">
                            <div>
                                ...
                                @if (Auth::guest())
                                    <div class='row'>
                                        <div class=" col-6  text-gray-900 dark:text-white">{!! $movie->likes()->count()!!}<i  class=" like dark:text-white fa fa-thumbs-up"></i></div>

                                        <div class="  text-gray-900 dark:text-white">{!! $movie->dislikes()->count()!!}<i  class=" dislike dark:text-white fa fa-thumbs-down"></i></div>
                                    </div>
                                @else
                                    ...
                                    <form action="/like/{{$movie->id}}" method="POST">
                                            ...
                                            <button><div class=" col-6  text-gray-900 dark:text-white">{!! $movie->likes()->count()!!}<i  class=" like dark:text-white fa fa-thumbs-up"></i></div></button>
                                        </form>
                                        <form action="/dislike/{{$movie->id}}" method="POST">
                                            @csrf
                                            
                                            <button> <div class="  text-gray-900 dark:text-white">{!! $movie->dislikes()->count()!!}<i  class=" dislike dark:text-white fa fa-thumbs-down"></i></div></button>
                                        </form>
                                    @endif
                                @endif
                                @can('like', $movie)
                                ...
                                @endcan
                                    ...
                                    <!-- <form action="/like/{{ $movie }}" method="POST">
                                        @csrf
                                        @method('DELETE')
